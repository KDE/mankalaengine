...
inline QString igisorodescription() {
    return i18n(
        ...
        "Players take turns selecting a hole with 2 or more seeds on their side"
        "and sowing seeds counter clockwise one by one into subsequent holes.\n"
        "If the last seed lands in an empty hole, the turn ends. If it lands "
        "in a"
        "non-empty hole, the player may continue sowing.\n"
        "Capturing is an important strategic element of Igisoro and depends on "
        "the"
        "position of seeds on both sides of the board, especially the "
        "opponent’s"
        "front row.  To capture, each player must have played at least two "
        "moves."
        "Capture is done when a seed lands on the inner row, the opposite holes"
        "on the opponents side must have seeds, the hole in the outer row must"
        "also have a seed.  The seeds on the opponents side are captured and"
        "play restarts from the initial hole from which the seeds that lead to"
        "the capture began.\n"
        "If sowing ends in the first or last hole on the outer row, or in the"
        "second or second to last hole on the inner row, if one has not"
        "captured and the hole contains seeds, one can choose to play in the"
        "clockwise direction.  If one chooses to play in the clockwise"
        "direction, one can still capture if the last seed sown lands in"
        "the inner row, the corresponding hole in the outer row contains"
        "seeds and both the corresponding rows on the opponents side contain"
        "seeds.\n"
        "In some situations, players may choose not to capture using 'ndahise',"
        "and the opponent may respond with 'ndakubye' and then take seeds from"
        "the hole in the inner row where they would have been captured to the"
        "outer row.\n"
        "The game ends when a player can no longer make a legal move, making"
        "that player the loser.\n"
        "This implementation represents a variant of Igisoro, other variants"
        "exist.\n");
}
} // namespace MankalaEngine
